...
template<class T> bool Albero<T>::alberovuoto() //restituisce true se l'Albero è vuoto, false altrimenti
{
    return cont == 0;
}

template<class T> void Albero<T>::inssottoalbero(nodo& nodp, nodo& nodf) //inserisce il nodo la cui posizione sarà identificata da due nodi
{
...
template<class T> void Albero<T>::cancnodo(nodo& nod) //cancella il nodo dato in input
{
    ...
    if(!alberovuoto()) //se l'Albero non è vuoto allora...
    {
...
template<class T> bool Albero<T>::finefratelli(nodo nod) //ritorna true se il nodo è l'ultimo dei fratelli, false altrimenti
{
    return nod == precnodo(primofratello(nod));
}

template<class T> bool Albero<T>::foglia(nodo nod) //ritorna true se il nodo è una foglia dell'Albero, false altrimenti
{
...
{
    if(!alberovuoto()) //se l'Albero non è vuoto...
    { //... procedo con la invisita:
        if(!foglia(nod)) //---------------------------------+
        { //                                                |
            cellaalbero<T>* temp = primofiglio(nod); //    +-> 1) se il nodo non è una foglia allora vado al primo figlio
            contanodi(temp); //                             |
        } //------------------------------------------------+
