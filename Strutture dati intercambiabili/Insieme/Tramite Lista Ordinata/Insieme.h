 ...
 * @note Definizione di Insieme per la realizzazione tramite lista ordinata.
...
private:
	ListaOrdinata<TipoElem> setlist;
};
...
Insieme<TipoElem>::Insieme(const Insieme<TipoElem>& i)
{
	 ...
	 * un costruttore di copia per le liste ordinate. Tale 
	 * supposizione non è casuale: il C++ ne richiede
	 * uno, per cui in sua assenza ne definisce uno
	 * di default.
	 */
	setlist = ListaOrdinata<TipoElem>(i.setlist);
}
...
bool Insieme<TipoElem>::appartiene(const TipoElem& e) const
{
    ...
    typename Lista<TipoElem>::posizione p = setlist.primolista();
    while (!setlist.finelista(p) && !trovato && continua)
    {
        if(setlist.leggilista(p) == e)
            trovato = true;
        else if(setlist.leggilista(p) <= e)
