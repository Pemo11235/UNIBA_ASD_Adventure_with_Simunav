...
public:
	...
	void set_key(const K&); // restituisce la chiave
...
template<typename K, typename V> void Entry<K, V>::set_key(const K &k) {
	chiave = k;
}

template<typename K, typename V> void Entry<K, V>::set_value(const V &v) {
	valore = v;
}
